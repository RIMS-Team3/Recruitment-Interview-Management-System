...
namespace RecruitmentInterviewManagementSystem.Infastructure.ServiceImplement
{
    public class EmailProducer : IEmailProducer
    {
        ...
        public EmailProducer(IConfiguration configuration, ILogger<EmailProducer> logger)
        {
        ...
        public async Task Execute(NotificationDTOS request)
        {
            ...
            await _channel.ExchangeDeclareAsync(_iConfig["RabbitMQ:Exchange:Email"]!, type: ExchangeType.Direct, durable: true);


            await _channel.QueueDeclareAsync(
                _iConfig["RabbitMQ:Queuen:Email"]!,
                durable: true,
                exclusive: false,
                autoDelete: false
            );
            ...
            await _channel.QueueBindAsync(
                queue: _iConfig["RabbitMQ:Queuen:Email"]!,
                exchange: _iConfig["RabbitMQ:Exchange:Email"]!,
                routingKey: _iConfig["RabbitMQ:Queuen:RoutingKey"]!
            );
